...
def run(ymir_cfg: edict, ymir_yolov5: YmirYolov5):
    ...
    for idx, batch in enumerate(pbar):
        ...
        with torch.no_grad():
            pred,proto = ymir_yolov5.forward(batch['image'].float().to(device), nms=[True,False])
    ...
    for idx, batch in enumerate(pbar):
        ...
        for key in aug_keys:
            with torch.no_grad():
                pred_result[key],proto = ymir_yolov5.forward(batch[f'image_{key}'].float().to(device), nms=[True,False])
