...
def run(
    ...
    for batch_i, (im, targets, paths, shapes, masks) in enumerate(pbar):
        ...
        for si, (pred, proto) in enumerate(zip(preds, protos)):
            ...
            gt_masks = masks[midx]
            masks_prob,pred_masks = process(proto, pred[:, 6:], pred[:, :4], shape=im[si].shape[1:])
    ...
    LOGGER.info(pf % ("all", seen, nt.sum(), *(metrics.mean_results()[:4]+metrics.mean_results()[7:11])))
    if nt.sum() == 0:
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(metrics.ap_class_index):
            LOGGER.info(pf % (names[c], seen, nt[c], *metrics.class_result(i)[:4]+metrics.class_result(i)[7:11]))
    ...
    mp_bbox, mr_bbox, map50_bbox, map_bbox, mtp_bbox, mfp_bbox, mfn_bbox,mp_mask, mr_mask, map50_mask, map_mask,mtp_mask, mfp_mask,mfn_mask = metrics.mean_results()
    ...
    print('mask_tp_fp',[mtp_mask, mfp_mask])
    return (*final_metric, *(loss.cpu() / len(dataloader)).tolist()), metrics.get_maps(nc), t,[mtp_bbox, mfp_bbox,mfn_bbox],[mtp_mask, mfp_mask,mfn_mask]
