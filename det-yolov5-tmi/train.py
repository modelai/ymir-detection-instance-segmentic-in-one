        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if best_fitness == fi:
                    ...
                    write_ymir_training_result(ymir_cfg, map50=float(best_fitness[0]), id=f'best', files=[str(best)])
                if (epoch > 0) and (opt.save_period > 0) and (epoch % opt.save_period == 0):
                    torch.save(ckpt, w / f'epoch{epoch}.pt')
                    weight_file = str(w / f'epoch{epoch}.pt')
                    write_ymir_training_result(ymir_cfg, map50=float(results[2]), id=f'epoch_{epoch}', files=[weight_file])
                del ckpt
